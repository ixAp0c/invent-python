...
def drawBoard(board):
    hline = '    ' # Initial space for numbers down left side of board
    for i in range(1, 6):
        hline += (' ' * 9) + str(i)
...
def getNewBoard():
    ...
    for x in range(60): # The main list is a list of 60 lists
        board.append([])
        for y in range(15): # Each list in the main list has 15 char strings
            if random.randint(0, 1) == 0: # Use different characters for the
                board[x].append('~')      # ocean to make it more readable.
            else:
                board[x].append('`')
    return board
...
# Return True if coordinates are on board, otherwise False.
def isValidMove(x, y):
    return x >= 0 and x <= 59 and y >= 0 and y <= 14

# Change the board data structures with a sonar device character.
# Remove treasure chests from the chests list as they are found. Return False
# if this is an invalid move. Otherwise, return the string of the result of
# this move.
...
treasurue chests up to a distance of 9. If all chests are out of
range, the point will be marked with 0.
...
while True:
    ...
    if sonarDevices == 0:
        ...
        print('and head for home with treasure chests still out there! Game over.')
